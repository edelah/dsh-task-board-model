...
var BoardController = class {
	...
	*/
	...
	openCodexConversation(id) {
		const task = this.tasks.find((candidate) => candidate.id === id);
		const latest = task?.executions[task.executions.length - 1];
		if (latest?.runner !== "codex" || latest.threadId === void 0) return;
		this.lastCurrent = currentOf(this.deps.sessions);
	...
	*/
	...
	handleExecutionEvent(event) {
		if (event.kind === "started") {
			this.tasks = this.tasks.map((task) => task.id === event.taskId ? attachRunIdentity(task, event.executionId, event.sessionId, event.runner ?? "dsh", event.runId, event.threadId, this.now()) : task);
			this.persistAndNotify();
			return;
		}
...
* Codex CLI process.
*/
function attachRunIdentity(task, executionId, sessionId, runner, runId, threadId, now) {
	return {
		...task,
		updatedAt: now,
