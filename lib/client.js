...
function latestCodexThreadId(task) {
	for (let index = task.executions.length - 1; index >= 0; index -= 1) {
		const execution = task.executions[index];
		if (execution.runner === "codex" && execution.threadId !== void 0) return execution.threadId;
	}
}
/**
...
var ExecutionService = class {
	...
	*/
	...
	followUpExecutions = /* @__PURE__ */ new Set();
	...
	async run(task, execution, onEvent) {
		...
		try {
			...
			const sessionId = await this.connectSession(runWorkspaceId, task.modelSelection !== void 0, runWorkspaceId !== task.workspaceId);
			onEvent({
				kind: "started",
				taskId: task.id,
				executionId: execution.id,
	...
	*/
	async runViaCodex(task, execution, onEvent, followUp) {
		const settleFailed = (error) => {
			onEvent({
				kind: "settled",
				taskId: task.id,
		...
		const resumeThreadId = isFollowUp ? previousThread : void 0;
		if (isFollowUp && previousThread === void 0) {
		...
		const started = await codex.start({
			...
			taskId: task.id,
			...resumeThreadId === void 0 ? {} : { resumeThreadId },
			...task.codexModel === void 0 ? {} : { model: task.codexModel },
			...task.codexEffort === void 0 ? {} : { effort: task.codexEffort },
			...task.permission === void 0 ? {} : { sandbox: task.permission }
		});
		if (!started.ok) {
		...
		onEvent({
			...
			runner: "codex",
			runId: started.runId,
			...started.threadId === void 0 ? {} : { threadId: started.threadId },
			...started.cwd === void 0 ? {} : { cwd: started.cwd }
		});
	...
	*/
	async resolveRunTarget(task, execution, onEvent, settleFailed) {
		const list = this.env.workspaces.list.getSnapshot();
		const pathOf = (workspaceId) => workspaceId === void 0 ? void 0 : list.items.find((item) => item.workspaceId === workspaceId)?.path;
		const basePath = pathOf(task.workspaceId) ?? pathOf(list.recentWorkspaceId) ?? list.items.find((item) => typeof item.path === "string" && item.path !== "")?.path;
		if (basePath === void 0 || basePath === "") {
			settleFailed("no workspace with a known path is available to run the task in");
			return;
		}
		const spec = task.worktree;
		if (spec === void 0) return { cwd: basePath };
		...
		try {
			...
			let workspaceId;
			if (this.env.registerWorkspace !== void 0) try {
	...
	*/
	async pollCodexRun(codex, taskId, executionId, runId, onEvent) {
		const intervalMs = this.env.pollIntervalMs ?? 2e3;
		for (;;) {
	...
	*/
	async runCodexFollowUp(task, execution, content, onEvent) {
	...
	canFollowUp(task) {
		return task.executor === "codex" && latestCodexThreadId(task) !== void 0;
	}
	/** Read the latest Codex thread for a task without changing its state. */
	async readCodexConversation(task) {
		const threadId = latestCodexThreadId(task);
		if (threadId === void 0) return {
			ok: false,
			error: "this task has no persisted Codex thread"
		};
		const codex = this.env.codex;
		if (codex === void 0) return {
			ok: false,
			error: "codex executor unavailable"
		};
		try {
	...
	async importCodexConversation(task) {
		const threadId = latestCodexThreadId(task);
		if (threadId === void 0) return {
			ok: false,
			error: "this task has no persisted Codex thread"
		...
		if (codex.importConversation === void 0) return {
			ok: false,
			error: "native DSH session bridge unavailable"
		};
		try {
			const cwd = task.executions[task.executions.length - 1]?.cwd ?? task.worktree?.path ?? this.workspacePath(task);
	...
	workspacePath(task) {
		...
		const pathOf = (workspaceId) => workspaceId === void 0 ? void 0 : items.find((item) => item.workspaceId === workspaceId)?.path;
	...
	*/
	async peekCodexRun(task) {
		const execution = task.executions[task.executions.length - 1];
		if (execution?.runner !== "codex") return void 0;
		const settled = execution.endedAt !== void 0;
		if (!settled && execution.runId !== void 0) {
			try {
	...
	async steerCodexRun(task, content) {
		const execution = task.executions[task.executions.length - 1];
		if (execution?.runner !== "codex" || execution.runId === void 0 || execution.endedAt !== void 0) return {
			ok: false,
			error: "no active codex run to steer"
		};
		const codex = this.env.codex;
		if (codex === void 0) return {
			ok: false,
	...
	*/
	async cancelCodexRun(task) {
		const execution = task.executions[task.executions.length - 1];
		if (execution?.runner !== "codex" || execution.runId === void 0 || execution.endedAt !== void 0) return false;
		const codex = this.env.codex;
		if (codex === void 0) return false;
		try {
	...
	*/
	async prepareWorktree(task, onReady) {
		if (task.worktree === void 0) return {
			ok: false,
			error: "the task has no worktree configured"
		};
		const list = this.env.workspaces.list.getSnapshot();
		const pathOf = (workspaceId) => workspaceId === void 0 ? void 0 : list.items.find((item) => item.workspaceId === workspaceId)?.path;
		const basePath = pathOf(task.workspaceId) ?? pathOf(list.recentWorkspaceId) ?? list.items.find((item) => typeof item.path === "string" && item.path !== "")?.path;
		if (basePath === void 0 || basePath === "") return {
			ok: false,
			error: "no workspace with a known path is available"
		};
		const worktrees = this.env.worktrees;
		if (worktrees === void 0) return {
			ok: false,
			error: "this deployment does not support git worktrees"
		};
		try {
			const ensured = await worktrees.ensure({
				repoPath: basePath,
	...
	async removeWorktree(path, force = false) {
		...
		try {
			const removed = await worktrees.remove({
				path,
				...force ? { force: true } : {}
			});
			return removed.ok ? { ok: true } : {
				ok: false,
				error: messageOf(removed.error)
			};
		} catch (error) {
			return {
				ok: false,
	...
	*/
	async reconcile(task) {
		const execution = task.executions[task.executions.length - 1];
		if (execution === void 0 || execution.endedAt !== void 0) return void 0;
		if (execution.runner === "codex") return this.reconcileCodex(task, execution);
		if (execution.sessionId === void 0) return void 0;
	...
	*/
	async reconcileCodex(task, execution) {
		const codex = this.env.codex;
		if (codex === void 0 || execution.runId === void 0) return {
			kind: "settled",
			taskId: task.id,
			executionId: execution.id,
			outcome: "cancelled",
			error: "the codex run can no longer be tracked"
		};
		let status;
		try {
		...
		if (status.state === "running") return void 0;
		...
		if (status.state === "interrupted") return {
			kind: "settled",
			taskId: task.id,
			executionId: execution.id,
			outcome: "cancelled",
	...
	async connectSession(taskWorkspaceId, dedicated, freshlyRegistered = false) {
		const workspace = this.env.workspaces.list.getSnapshot();
		if (taskWorkspaceId !== void 0 && taskWorkspaceId !== "") {
			if (!freshlyRegistered && !workspace.items.some((item) => item.workspaceId === taskWorkspaceId)) throw new Error(`task workspace is not available: ${taskWorkspaceId}`);
...
const TASK_EXECUTORS = ["dsh", "codex"];
/** Whether an unknown value is a known executor id. */
function isTaskExecutor(value) {
	return typeof value === "string" && TASK_EXECUTORS.includes(value);
}
...
function normalizeWorktree(value) {
	...
	const path = typeof spec.path === "string" && spec.path.trim() !== "" ? spec.path.trim() : void 0;
	const workspaceId = typeof spec.workspaceId === "string" && spec.workspaceId.trim() !== "" ? spec.workspaceId.trim() : void 0;
	const createdAt = typeof spec.createdAt === "number" && Number.isFinite(spec.createdAt) ? spec.createdAt : void 0;
	return {
		branch,
		...path === void 0 ? {} : { path },
		...workspaceId === void 0 ? {} : { workspaceId },
...
function createTask(input, now, id) {
	return {
		...
		executor: isTaskExecutor(input.executor) && input.executor !== "dsh" ? input.executor : void 0,
		codexModel: normalizeTargetId(input.codexModel),
		codexEffort: normalizeTargetId(input.codexEffort),
		worktree: input.worktree === void 0 ? void 0 : normalizeWorktree({ branch: input.worktree.branch })
	};
}
...
*/
function settleExecution(task, executionId, outcome, now, error, outputTail) {
...
*/
function normalizeExecutor(current, value) {
	if (value === void 0) return void 0;
	if (!isTaskExecutor(value)) return current;
	return value === "dsh" ? void 0 : value;
}
/**
...
function applyUpdateTask(tasks, id, patch, now) {
	return tasks.map((task) => {
		...
		const executor = "executor" in patch ? normalizeExecutor(task.executor, patch.executor) : void 0;
		const codexModel = "codexModel" in patch ? normalizeTargetId(patch.codexModel) : void 0;
		const codexEffort = "codexEffort" in patch ? normalizeTargetId(patch.codexEffort) : void 0;
		const worktree = "worktree" in patch ? normalizeWorktree(patch.worktree) : void 0;
		const next = {
			...task,
			...patch,
			updatedAt: now
		...
		if (executor !== void 0 || "executor" in patch) next.executor = executor;
		if (codexModel !== void 0 || "codexModel" in patch) next.codexModel = codexModel;
		if (codexEffort !== void 0 || "codexEffort" in patch) next.codexEffort = codexEffort;
...
function selectedCodexTaskOf(snapshot) {
	if (snapshot.codexChatTaskId === void 0) return void 0;
...
var BoardController = class {
	...
	openBoard() {
		const leavingChat = this.codexChatTaskId !== void 0;
		this.codexChatTaskId = void 0;
		if (this.boardOpen) {
	...
	toggleBoard() {
		if (this.boardOpen && this.codexChatTaskId === void 0) this.closeBoard();
		else this.openBoard();
	}
	/**
	...
	*/
	async openCodexConversation(id) {
		const task = this.tasks.find((candidate) => candidate.id === id);
		const latest = task?.executions[task.executions.length - 1];
		if (task === void 0 || latest?.runner !== "codex" || latest.threadId === void 0) return;
		if (latest.sessionId !== void 0) {
			this.closeBoard();
		...
		if (fresh === void 0 || execution === void 0 || execution.runner !== "codex") return;
		this.tasks = this.tasks.map((candidate) => candidate.id === id ? attachRunIdentity(candidate, execution.id, imported.sessionId, "codex", void 0, void 0, execution.cwd, this.now()) : candidate);
	...
	*/
	...
	deleteTask(id) {
		...
		if (this.codexChatTaskId === id) this.codexChatTaskId = void 0;
		this.persistAndNotify();
	}
	/**
	...
	*/
	openTaskWorkspace(id) {
		const workspaceId = this.tasks.find((candidate) => candidate.id === id)?.worktree?.workspaceId;
		if (workspaceId === void 0 || this.deps.openWorkspace === void 0) return;
	...
	*/
	...
	handleExecutionEvent(event) {
		if (event.kind === "started") {
			this.tasks = this.tasks.map((task) => task.id === event.taskId ? attachRunIdentity(task, event.executionId, event.sessionId, event.runner ?? "dsh", event.runId, event.threadId, event.cwd, this.now()) : task);
			this.persistAndNotify();
			return;
		}
		if (event.kind === "worktree-ready") {
			this.adoptWorktree(event.taskId, event.path, event.branch, event.workspaceId);
			return;
		}
		this.activeExecutionIds.delete(event.executionId);
		this.tasks = this.tasks.map((task) => task.id === event.taskId ? settleExecution(task, event.executionId, event.outcome, this.now(), event.error, event.outputTail) : task);
		this.persistAndNotify();
	}
	/**
	...
	*/
	async adoptWorktree(taskId, path, branch, knownWorkspaceId) {
		const task = this.tasks.find((candidate) => candidate.id === taskId);
		if (task === void 0) return;
		let workspaceId = knownWorkspaceId ?? task.worktree?.workspaceId;
		if (workspaceId === void 0 && this.deps.registerWorkspace !== void 0) try {
			const registered = await this.deps.registerWorkspace(path, task.title);
			if (registered !== void 0) workspaceId = registered;
		} catch (error) {
			console.error("[dsh-task-board] worktree workspace registration failed", error);
		}
		...
		const spec = {
			...
			path,
			...workspaceId === void 0 ? {} : { workspaceId },
			createdAt: fresh.worktree?.createdAt ?? this.now()
		};
	...
	*/
	async prepareWorktree(id) {
		...
		const result = await this.deps.exec.prepareWorktree(task, ({ path, branch, workspaceId }) => {
			this.adoptWorktree(task.id, path, branch, workspaceId);
		});
		return result.ok ? {
			ok: true,
			path: result.path
	...
	*/
	async removeWorktree(id, force = false) {
		const task = this.tasks.find((candidate) => candidate.id === id);
		const path = task?.worktree?.path;
		if (task === void 0 || path === void 0) return false;
		if (!(await this.deps.exec.removeWorktree(path, force)).ok) return false;
		const workspaceId = task.worktree?.workspaceId;
		if (workspaceId !== void 0 && this.deps.deleteWorkspace !== void 0) try {
			await this.deps.deleteWorkspace(workspaceId);
		} catch (error) {
			console.error("[dsh-task-board] worktree workspace deletion failed", error);
		}
		this.updateTask(id, { worktree: void 0 });
		return true;
	}
	/**
	...
	*/
	async followUpTask(id, content) {
		...
		this.persistAndNotify();
		this.activeExecutionIds.add(execution.id);
		await this.deps.exec.runCodexFollowUp(next, execution, content.trim(), (event) => {
			this.handleExecutionEvent(event);
		});
		return true;
	}
	...
	*/
	async steerExecution(id, content) {
		const task = this.tasks.find((candidate) => candidate.id === id);
		if (task === void 0 || content.trim() === "") return {
			ok: false,
...
* Codex CLI process.
*/
function attachRunIdentity(task, executionId, sessionId, runner, runId, threadId, cwd, now) {
	return {
		...task,
		updatedAt: now,
...
function isTaskRecordShape(value) {
	...
	for (const execution of record.executions) {
		...
		if (entry.runner !== void 0 && entry.runner !== "dsh" && entry.runner !== "codex") return false;
		if (entry.runId !== void 0 && typeof entry.runId !== "string") return false;
		if (entry.cwd !== void 0 && typeof entry.cwd !== "string") return false;
		if (entry.threadId !== void 0 && typeof entry.threadId !== "string") return false;
		if (entry.outputTail !== void 0 && typeof entry.outputTail !== "string") return false;
	}
	return true;
}
...
function parseLedger(raw) {
	...
	for (const row of parsed) {
		...
		task.executor = isTaskExecutor(row.executor) && row.executor !== "dsh" ? row.executor : void 0;
...
const zh = {
	"entry.label": "任务看板",
	"job.running": "运行中 · {name}",
	"job.succeeded": "已完成 · {name}",
	"job.failed": "已失败 · {name}",
	"job.cancelled": "已取消 · {name}",
	"board.title": "任务看板",
	"board.close": "返回会话",
	"board.new": "新建任务",
	"board.search": "筛选任务…",
	...
	"chat.taskSettings": "任务设置",
	"chat.loading": "正在加载 Codex 对话…",
	"chat.retry": "重试",
	"chat.activity": "活动详情",
	"chat.empty": "这个 Codex 线程还没有消息。",
	"chat.fallback": "无法读取完整线程，以下是保存的输出。",
	"chat.running": "Codex 正在处理",
	...
	"settings.enabledHint": "关闭后隐藏侧边栏入口、执行任务条目与看板视图。",
...
const en = {
	...
	"job.succeeded": "Succeeded · {name}",
	"job.failed": "Failed · {name}",
	"job.cancelled": "Cancelled · {name}",
	...
	"chat.taskSettings": "Task settings",
	"chat.loading": "Loading Codex conversation…",
	"chat.retry": "Retry",
	"chat.activity": "Activity details",
	"chat.empty": "This Codex thread has no messages yet.",
	"chat.fallback": "The full thread could not be read. Showing the saved output instead.",
	"chat.running": "Codex is working",
	...
	"settings.enabledHint": "When off, the sidebar entry, execution rows, and board view are hidden.",
...
var board_module_default = {
	"activityList": "JEp9Zq_activityList",
	"activityTime": "JEp9Zq_activityTime",
	...
	"codexActivity": "JEp9Zq_codexActivity",
	"codexAssistantMessage": "JEp9Zq_codexAssistantMessage",
	"codexChat": "JEp9Zq_codexChat",
	"codexChatError": "JEp9Zq_codexChatError",
	"codexChatHeader": "JEp9Zq_codexChatHeader",
	"codexChatMessages": "JEp9Zq_codexChatMessages",
	"codexChatNotice": "JEp9Zq_codexChatNotice",
	"codexChatTitle": "JEp9Zq_codexChatTitle",
	"codexChatTitleGroup": "JEp9Zq_codexChatTitleGroup",
	"codexCommentaryMessage": "JEp9Zq_codexCommentaryMessage",
	"codexComposer": "JEp9Zq_codexComposer",
	"codexComposerInput": "JEp9Zq_codexComposerInput",
	"codexRunningLabel": "JEp9Zq_codexRunningLabel",
	"codexTurn": "JEp9Zq_codexTurn",
	"codexTurnError": "JEp9Zq_codexTurnError",
	"codexUserMessage": "JEp9Zq_codexUserMessage",
	...
	"executorChip": "JEp9Zq_executorChip",
	"field": "JEp9Zq_field",
	"fieldLabel": "JEp9Zq_fieldLabel",
	"followUpInput": "JEp9Zq_followUpInput",
	"followUpRow": "JEp9Zq_followUpRow",
	...
	"outputBlock": "JEp9Zq_outputBlock",
	...
	"worktreeMeta": "JEp9Zq_worktreeMeta",
	"worktreePath": "JEp9Zq_worktreePath",
	"worktreeRow": "JEp9Zq_worktreeRow"
};
//#endregion
...
//#endregion
//#region src/client/board/CodexConversation.tsx
/** Chat-first view of one persisted Codex App Server thread. */
function CodexConversation({ controller, task }) {
	const latest = task.executions[task.executions.length - 1];
	const running = task.status === "running" && latest?.runner === "codex" && latest.endedAt === void 0;
	...
	const load = (0, react.useCallback)(() => {
		...
		controller.codexConversation(task.id).then((result) => {
	...
	(0, react.useEffect)(() => {
		...
		let alive = true;
		const tick = () => {
			controller.codexRunSnapshot(task.id).then((snapshot) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: board_module_default.codexChat,
		"data-dsh-taskboard-codex-chat": "",
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
				className: board_module_default.codexChatHeader,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						children: ["‹ ", t("board.backToBoard")]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: board_module_default.codexChatTitleGroup,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
							className: board_module_default.codexChatTitle,
							children: task.title
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						...
						children: t("chat.taskSettings")
					})
				]
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("main", {
				className: board_module_default.codexChatMessages,
				children: [
					state.kind === "loading" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: board_module_default.codexChatNotice,
						children: t("chat.loading")
					}),
					state.kind === "error" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						className: board_module_default.codexChatError,
						children: [
							...
							savedTail !== void 0 && savedTail !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: board_module_default.codexChatNotice,
								children: t("chat.fallback")
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: board_module_default.codexAssistantMessage,
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.MarkdownText, { text: savedTail })
							})] })
						]
					}),
					state.kind === "ready" && !hasMessages && !running && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: board_module_default.codexChatNotice,
						children: t("chat.empty")
					}),
					turns.map((turn) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						className: board_module_default.codexTurn,
						"data-status": turn.status,
						children: [
							turn.messages.map((message) => message.role === "user" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: board_module_default.codexUserMessage,
								children: message.text
							}, message.id) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: message.phase === "commentary" ? board_module_default.codexCommentaryMessage : board_module_default.codexAssistantMessage,
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.MarkdownText, { text: message.text })
							}, message.id)),
							turn.activity.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
								className: board_module_default.codexActivity,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", { children: [
									t("chat.activity"),
									" · ",
									turn.activity.length
								] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", { children: turn.activity.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
									"data-kind": item.kind,
									children: item.text
								}, item.id)) })]
							}),
							turn.error !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: board_module_default.codexTurnError,
								children: turn.error
							})
						]
					}, turn.id)),
					running && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
							className: board_module_default.codexRunningLabel,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: board_module_default.cardSpinner }),
								" ",
								t("chat.running")
							]
						}), live?.liveAnswer !== void 0 && live.liveAnswer.trim() !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: board_module_default.codexAssistantMessage,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.MarkdownText, {
								text: live.liveAnswer,
								streaming: true
							})
						})]
					}),
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
				className: board_module_default.codexComposer,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
						className: board_module_default.codexComposerInput,
						rows: 2,
						value: followUp,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						...
						children: sending ? t("detail.sending") : t("detail.send")
					}),
...
//#endregion
//#region src/client/board/ExecutorFields.tsx
/** Sentinel select value for the "custom slug" choice. */
const CODEX_MODEL_CUSTOM = "__custom__";
/**
...
* not know about.
*/
function CodexModelField({ options, value, onChange }) {
	const known = value !== void 0 && options.models.some((model) => model.slug === value);
	const custom = value !== void 0 && value !== "" && !known;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			...
			onChange: (event) => {
				...
				onChange(next === "" ? void 0 : next);
			},
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value: "",
					children: options.defaultModel !== void 0 ? t("exec.codexModel.default", { value: options.defaultModel }) : t("exec.codexModel.defaultPlain")
				}),
				!known && custom && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value: "__custom__",
					children: value
				}),
				options.models.map((model) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value: model.slug,
					children: model.displayName
				}, model.slug)),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value: CODEX_MODEL_CUSTOM,
					children: t("exec.codexModel.custom")
				})
			]
		})]
	}), custom && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
		className: board_module_default.field,
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			className: board_module_default.fieldLabel,
			children: t("exec.codexModel.customLabel")
			...
			onChange: (event) => {
				onChange(event.target.value === "" ? void 0 : event.target.value);
			}
		})]
	})] });
}
/** The Codex reasoning-effort picker over the selected model's levels. */
function CodexEffortField({ options, modelSlug, value, onChange }) {
	...
	const known = value !== void 0 && (model === void 0 || model.efforts.some((effort) => effort.id === value));
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
		className: board_module_default.field,
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			className: board_module_default.fieldLabel,
			children: t("new.codexEffort")
		}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
			className: board_module_default.select,
			value: value ?? "",
			onChange: (event) => {
			...
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value: "",
					children: defaultEffort !== void 0 ? t("exec.codexEffort.default", { value: defaultEffort }) : t("exec.codexEffort.defaultPlain")
				}),
				!known && value !== void 0 && value !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
					value,
					children: value
				}),
...
//#endregion
//#region src/client/board/WorktreeSection.tsx
/**
...
function WorktreeCreateFields({ enabled, branch, onEnabledChange, onBranchChange, invalid }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: board_module_default.field,
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
			...
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("worktree.enable") })]
		}), enabled && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
			className: `${board_module_default.input}${invalid ? ` ${board_module_default.scheduleInputInvalid}` : ""}`,
			value: branch,
			...
			children: t("worktree.invalidBranch")
		})] })]
	});
}
/** The detail view's worktree section: state + prepare / open / remove. */
function WorktreeDetailSection({ controller, task }) {
	const spec = task.worktree;
	const [branchDraft, setBranchDraft] = (0, react.useState)(spec?.branch ?? "");
	...
	const [recorded, setRecorded] = (0, react.useState)(spec);
	if (recorded !== spec) {
		setRecorded(spec);
	...
	const saveBranch = () => {
		...
		setInvalid(false);
		setError(void 0);
		controller.updateTask(task.id, { worktree: { branch: trimmed === "" ? slugifyBranch(task.title, Date.now()) : trimmed } });
	};
	...
	const remove = async (force) => {
		...
		setError(void 0);
		if (!await controller.removeWorktree(task.id, force)) setError(t("worktree.prepareFailed", { error: t("worktree.remove") }));
	};
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: board_module_default.detailSection,
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: t("detail.worktree") }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
					...
					disabled: running,
					onChange: (event) => {
						controller.updateTask(task.id, { worktree: event.target.checked ? { branch: slugifyBranch(task.title, Date.now()) } : void 0 });
					}
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("worktree.enable") })]
			}),
			spec !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: board_module_default.scheduleRow,
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						className: `${board_module_default.input} ${board_module_default.scheduleInput}${invalid ? ` ${board_module_default.scheduleInputInvalid}` : ""}`,
						value: branchDraft,
						placeholder: t("worktree.branchPlaceholder"),
						spellCheck: false,
						"aria-label": t("worktree.branch"),
				...
				spec.path !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
					className: board_module_default.worktreeMeta,
					children: [
						t("worktree.status.readyLabel"),
						" ",
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: board_module_default.worktreePath,
							children: spec.path
						})
					]
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: board_module_default.worktreeMeta,
					children: t("worktree.status.pending")
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: board_module_default.worktreeRow,
					children: [
						spec.path === void 0 && !running && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: board_module_default.ghostButton,
							disabled: preparing,
							onClick: () => {
							...
							children: preparing ? t("worktree.preparing") : t("worktree.prepare")
						}),
						spec.workspaceId !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: board_module_default.ghostButton,
							onClick: () => {
								controller.openTaskWorkspace(task.id);
							},
							children: t("worktree.openWorkspace")
						}),
						spec.path !== void 0 && !running && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
			...
			confirmRemove && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ConfirmDialog, {
...
function NewTaskModal({ controller, onClose }) {
	...
	const [executor, setExecutor] = (0, react.useState)("dsh");
	const [codexModel, setCodexModel] = (0, react.useState)(void 0);
	const [codexEffort, setCodexEffort] = (0, react.useState)(void 0);
	const [worktreeEnabled, setWorktreeEnabled] = (0, react.useState)(false);
	const [worktreeBranch, setWorktreeBranch] = (0, react.useState)("");
	...
	const submit = () => {
		...
		if (controller.createTask({
			...
			modelSelection: selectedModel === void 0 || codexSelected ? void 0 : {
				provider: selectedModel.provider,
				model: selectedModel.model,
				...reasoningEffort === "" ? {} : { reasoningEffort }
			},
			...worktreeEnabled ? { worktree: { branch: worktreeBranch.trim() !== "" ? worktreeBranch.trim() : slugifyBranch(title, Date.now()) } } : {},
			executor: codexSelected ? "codex" : "dsh",
			...codexSelected && codexModel !== void 0 ? { codexModel } : {},
			...codexSelected && codexEffort !== void 0 ? { codexEffort } : {},
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
							value: "dsh",
							children: t("exec.executor.dsh")
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
							value: "codex",
							children: [t("exec.executor.codex"), codex.available ? "" : t("exec.executor.codexUnavailable")]
						})]
					})]
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				...
				!codexSelected && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						className: board_module_default.field,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: board_module_default.fieldLabel,
				...
				codexSelected && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexModelField, {
						options: codex,
						value: codexModel,
						onChange: setCodexModel
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexEffortField, {
						options: codex,
						modelSlug: codexModel ?? codex.defaultModel,
						value: codexEffort,
						onChange: setCodexEffort
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: board_module_default.worktreeMeta,
						children: t("exec.hint.codex")
					})
				] }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					className: board_module_default.detailSection,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: t("new.worktree") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorktreeCreateFields, {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				...
				children: [
					...
					task.executor === "codex" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: board_module_default.executorChip,
						"data-executor": "codex",
						children: t("card.codex")
					}),
					task.worktree !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: board_module_default.executorChip,
						title: task.worktree.path ?? task.worktree.branch,
						children: t("card.worktree")
					}),
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				...
				children: (execution.runner ?? "dsh") === "codex" ? t("detail.runner.codex") : t("detail.runner.dsh")
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
			...
			execution.outputTail !== void 0 && execution.outputTail !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
				className: board_module_default.outputBlock,
				children: execution.outputTail
			})
		]
	});
}
...
function ExecutionSettingsSection({ controller, task }) {
	...
	const executor = isTaskExecutor(task.executor) && task.executor === "codex" ? "codex" : "dsh";
	const codexSelected = executor === "codex";
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				...
				children: codexSelected ? t("exec.hint.codex") : t("exec.hint")
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				className: board_module_default.field,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					onChange: (event) => {
						const next = event.target.value;
						controller.updateTask(task.id, { executor: isTaskExecutor(next) ? next : void 0 });
					},
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
						value: "dsh",
						children: t("exec.executor.dsh")
						...
						value: "codex",
						children: [t("exec.executor.codex"), options.codex.available ? "" : t("exec.executor.codexUnavailable")]
					})]
				})]
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
			...
			codexSelected && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexModelField, {
				options: options.codex,
				value: task.codexModel,
				onChange: (slug) => {
					controller.updateTask(task.id, { codexModel: slug });
				}
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexEffortField, {
				options: options.codex,
				...
				onChange: (effort) => {
					controller.updateTask(task.id, { codexEffort: effort });
				}
			})] })
		]
	});
...
*/
function CodexLiveSection({ controller, task }) {
	const latest = task.executions[task.executions.length - 1];
	const isCodex = task.executor === "codex";
	const running = task.status === "running" && latest?.runner === "codex" && latest?.endedAt === void 0;
	const canFollow = controller !== void 0 && !running && isCodex && latest?.threadId !== void 0;
	const [snapshot, setSnapshot] = (0, react.useState)(void 0);
	const [steerText, setSteerText] = (0, react.useState)("");
	const [followUp, setFollowUp] = (0, react.useState)("");
	const [sending, setSending] = (0, react.useState)(false);
	...
	(0, react.useEffect)(() => {
		...
		let alive = true;
		const tick = () => {
			controller.codexRunSnapshot(task.id).then((view) => {
	...
	const sendFollowUp = async () => {
		...
		setNotice(void 0);
		try {
			if (await controller.followUpTask(task.id, content)) setFollowUp("");
			else setNotice(t("detail.followUpUnavailable"));
		} finally {
			setSending(false);
		}
	};
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: board_module_default.detailSection,
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: t("detail.activity") }),
			activity.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
				className: board_module_default.activityList,
				children: [...activity].slice(-12).map((entry, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: board_module_default.activityTime,
					children: formatTime(entry.at)
				}), entry.text] }, `${entry.at}-${index}`))
			}),
			running && liveAnswer !== void 0 && liveAnswer.trim() !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: board_module_default.worktreeMeta,
				children: t("detail.liveAnswer")
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
				className: board_module_default.outputBlock,
				children: liveAnswer
			})] }),
			running && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: board_module_default.followUpRow,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
					className: board_module_default.input,
					value: steerText,
					...
					children: t("detail.steer")
				})]
			}),
			!running && canFollow && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				className: board_module_default.field,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: board_module_default.fieldLabel,
					children: t("detail.followUp")
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
					className: `${board_module_default.input} ${board_module_default.followUpInput}`,
					rows: 2,
					value: followUp,
					placeholder: t("detail.followUpPlaceholder"),
					onChange: (event) => {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					...
					className: board_module_default.worktreeMeta,
					children: t("detail.followUpHint")
				})]
			})] }),
			notice !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: board_module_default.formError,
...
function TaskDetail({ controller, task }) {
	...
	const running = task.status === "running";
	const latest = task.executions[task.executions.length - 1];
	const canStop = running && latest?.runner === "codex" && latest.endedAt === void 0;
	const [latestTask, setLatestTask] = (0, react.useState)(task);
	(0, react.useEffect)(() => {
		setLatestTask(task);
	}, [task]);
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorktreeDetailSection, {
							controller,
							task: current
						}),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexLiveSection, {
							controller,
							task: current
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
...
function TaskBoard({ controller }) {
	...
	if (codexTask !== void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodexConversation, {
		controller,
		task: codexTask
	});
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
...
function mountBoard(controller) {
	...
	const ensure = () => {
		...
		if (container !== void 0) {
			if (container.parentElement !== column || !container.isConnected) column.appendChild(container);
			return;
		}
...
function createJobRow(controller, taskId) {
	const el = document.createElement("button");
	el.type = "button";
	el.dataset.dshTaskboardJob = taskId;
	el.className = board_module_default.entry;
	const icon = document.createElement("span");
	icon.className = board_module_default.entryIcon;
	const spinner = document.createElement("span");
	spinner.className = board_module_default.cardSpinner;
	spinner.setAttribute("aria-hidden", "true");
	icon.appendChild(spinner);
	const label = document.createElement("span");
	label.className = board_module_default.entryLabel;
	...
	el.addEventListener("click", () => {
		...
		if (sessionId !== void 0) {
			controller.closeBoard();
			controller.openSession(sessionId);
		} else if (task?.executions[task.executions.length - 1]?.runner === "codex" && task.executions[task.executions.length - 1]?.threadId !== void 0) controller.openCodexConversation(taskId);
		else {
			controller.openBoard();
...
function paintRow(row, task, active) {
	const state = jobState(task);
	...
	} else if ("active" in row.el.dataset) delete row.el.dataset.active;
}
/**
...
*/
function workspaceGroup(root, workspaceId, workspaceIds) {
	const index = workspaceIds.indexOf(workspaceId);
	if (index < 0) return void 0;
	return Array.from(root.querySelectorAll("[class*=\"groupSection\"]"))[index];
}
/** Resolve the workspace where the runner actually places this task. */
function taskWorkspaceId(task, workspaces) {
	return task.worktree?.workspaceId ?? task.workspaceId ?? workspaces.recentWorkspaceId ?? workspaces.workspaceIds[0];
}
/** Whether this run still needs the plugin's synthetic sidebar row. */
function needsSyntheticRow(task) {
	if (task.archivedAt !== void 0) return false;
	const latest = task.executions[task.executions.length - 1];
	if (task.status === "running") return latest?.sessionId === void 0;
	if (latest?.endedAt === void 0) return false;
	return true;
}
/**
...
* settled geometry.
*/
function placeRows(root, groupedRows, workspaceIds) {
	for (const [workspaceId, rows] of groupedRows) {
		const group = workspaceGroup(root, workspaceId, workspaceIds);
		const header = group?.querySelector("[class*=\"projectRow\"]");
		if (group === void 0 || header === null || header === void 0) continue;
		let cursor = header.parentElement?.parentElement === group ? header.parentElement : header;
		for (const row of rows) {
			if (row.el.parentElement !== group || cursor.nextSibling !== row.el) group.insertBefore(row.el, cursor.nextSibling);
			cursor = row.el;
		}
	}
}
...
*/
function mountRunningJobs(controller, readWorkspaces = () => ({ workspaceIds: controller.getSnapshot().executionOptions.workspaces.map((workspace) => workspace.workspaceId) })) {
	if (typeof document !== "undefined" && document.querySelector("[data-dsh-taskboard-job]") !== null) return () => {};
	...
	*/
	const sync = () => {
		...
		for (const [taskId, row] of [...rows]) if (!visibleTasks.some((task) => task.id === taskId)) {
			row.el.remove();
			rows.delete(taskId);
		}
		for (const task of visibleTasks) if (!rows.has(task.id)) rows.set(task.id, createJobRow(controller, task.id));
		...
		for (const task of visibleTasks) {
			...
			if (group === void 0) groupedRows.set(workspaceId, [row]);
			else group.push(row);
		}
...
async function postHostRoute(path, body) {
	try {
	...
	const mountUi = () => {
		...
		const codexFace = {
			start: async (request) => {
				...
				if (payload.ok === true && typeof payload.runId === "string") return {
					ok: true,
					runId: payload.runId,
				...
				return {
					ok: false,
					error: payload.error ?? "codex start failed"
				};
			},
			status: async (runId) => {
				...
				if (payload.ok !== true) return {
					ok: false,
					error: payload.error ?? "codex status failed"
				};
				const activity = Array.isArray(payload.activity) ? payload.activity : void 0;
				...
				if (payload.state === "succeeded") return {
					ok: true,
					state: "succeeded",
					...threadId === void 0 ? {} : { threadId },
					...
					...activity === void 0 ? {} : { activity },
					...typeof payload.usage === "object" && payload.usage !== null ? { usage: payload.usage } : {}
				};
				if (payload.state === "interrupted") return {
					ok: true,
					state: "interrupted",
					...threadId === void 0 ? {} : { threadId },
					...typeof payload.outputTail === "string" ? { outputTail: payload.outputTail } : {}
			...
			readConversation: async (taskId, threadId) => {
				const payload = await postHostRoute(ROUTE_CODEX_THREAD, {
					taskId,
					threadId
				});
				...
				if (payload.ok === true && typeof conversation === "object" && conversation !== null) {
					...
					if (record.threadId === threadId && Array.isArray(record.turns)) return {
						ok: true,
						conversation
					};
				}
				return {
					ok: false,
					error: String(payload.error ?? "codex thread could not be read")
				};
			},
			importConversation: async (taskId, threadId, cwd) => {
				const payload = await postHostRoute(ROUTE_CODEX_IMPORT, {
					taskId,
					threadId,
					...cwd === void 0 ? {} : { cwd }
				});
				if (payload.ok === true && typeof payload.sessionId === "string" && payload.sessionId !== "") return {
					ok: true,
					sessionId: payload.sessionId
				};
				return {
					ok: false,
					error: payload.error ?? "codex thread could not be imported as a DSH session"
				};
			},
			steer: async (runId, content) => {
				const payload = await postHostRoute(ROUTE_CODEX_STEER, {
					runId,
					content
				});
				...
				return {
					ok: false,
					error: String(payload.error ?? "steer rejected")
				};
			},
			cancel: async (runId) => {
		...
		const worktreeFace = {
				...
				if (payload.ok === true && typeof payload.path === "string" && typeof payload.branch === "string") return {
					ok: true,
					path: payload.path,
				...
				return {
					ok: false,
					error: payload.error ?? "git worktree could not be created"
				};
			},
			remove: async (request) => {
				const payload = await postHostRoute(ROUTE_WORKTREE_REMOVE, request);
				return payload.ok === true ? { ok: true } : {
					ok: false,
					error: payload.error ?? "git worktree removal failed"
				};
			}
		};
		const registerWorkspace = async (path, title) => {
			const view = await workspaces.create({ path });
			if (view.title !== title && title.trim() !== "") try {
				return (await workspaces.rename(view.workspaceId, title)).workspaceId;
			} catch {
		...
		const controller = new BoardController({
			...
			openWorkspace: (workspaceId) => {
				workspaces.connectWorkspace(workspaceId).then((sessionId) => {
					sessions.open(sessionId);
				}, (error) => {
					console.error("[dsh-task-board] open workspace failed", error);
				});
			}
		});
		controller.start();
		...
		const pushCodexOptions = async () => {
			...
			for (const model of models) {
				...
				const efforts = Array.isArray(record.efforts) ? record.efforts.flatMap((effort) => {
					if (typeof effort !== "object" || effort === null) return [];
					const effortRecord = effort;
				...
				choices.push({
					slug: record.slug,
					displayName: typeof record.displayName === "string" && record.displayName !== "" ? record.displayName : record.slug,
					...typeof record.description === "string" ? { description: record.description } : {},
					efforts,
					...typeof record.defaultEffort === "string" ? { defaultEffort: record.defaultEffort } : {}
				});
			}
			controller.setExecutionOptions({ codex: {
				available: payload.available === true,
				models: choices,
				...typeof payload.defaultModel === "string" && payload.defaultModel !== "" ? { defaultModel: payload.defaultModel } : {},
				...typeof payload.defaultEffort === "string" && payload.defaultEffort !== "" ? { defaultEffort: payload.defaultEffort } : {}
			} });
		};
		pushCodexOptions();
		...
		try {
			...
			disposers.push(mountRunningJobs(controller, () => {
				...
				return {
					workspaceIds: workspaceList.items.map((item) => item.workspaceId),
					recentWorkspaceId: workspaceList.recentWorkspaceId
				};
			}));
