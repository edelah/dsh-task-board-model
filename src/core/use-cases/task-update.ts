 ...
 */
import { isTaskPermission, normalizeModelSelection, normalizeTargetId, type TaskRecord, type TaskPermission } from '../tasks.ts'

/** Editable fields on a task (the update patch surface). */
export type TaskUpdatePatch = Partial<Pick<TaskRecord, 'title' | 'description' | 'prompt' | 'workspaceId' | 'mode' | 'permission' | 'modelSelection'>>
